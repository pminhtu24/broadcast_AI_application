...
class QuoteGenerator:
    ...
    def _add_recipient(self, doc: Document, customer_name: str, customer_address: Optional[str], price_list: str):
        p = doc.add_paragraph()
        p.add_run(f"Kính gửi: {customer_name}")

        if customer_address:
            p2 = doc.add_paragraph()
            p2.add_run(customer_address)

        if price_list == "413":
            p3 = doc.add_paragraph()
            p3.add_run("Lời đầu tiên xin chân thành cảm ơn Quý cơ quan, chúng tôi xin được gửi báo giá hỗ trợ tuyên truyền như sau:")
    ...
    def _add_contract_info(self, doc: Document, contract_start: Optional[str], contract_end: Optional[str]):
        ...
        if len(doc.paragraphs) > 0:
            ...
            if last_p != p:
                ...
                p2 = doc.add_paragraph()
                p2.add_run("- Thời lượng: 60 giây/1 lần phát sóng")

    def _add_service_intro(self, doc: Document, price_list: str):
        if price_list == "413":
            p = doc.add_paragraph()
            p.add_run("Chúng tôi xin được gửi báo giá hỗ trợ tuyên truyền như sau:")
        else:
            p = doc.add_paragraph()
            p.add_run("Chúng tôi xin được gửi báo giá chi tiết quảng cáo trên kênh truyền hình THP của Báo và phát thanh, truyền hình Hải Phòng như sau:")
    ...
    def _add_table(self, doc: Document, items: List[Dict[str, Any]], price_list: str):
        if price_list == "413":
            ...
            headers = ["Khung giờ", "Số lần phát sóng", "Đơn giá", "Thành tiền"]
            cols = 4
        ...
        for item in items:
            ...
            if price_list == "413":
                ...
                row.cells[0].text = item.get("frame", "")
                row.cells[1].text = str(item.get("quantity", ""))
                row.cells[2].text = format_currency(item.get("unit_price", 0))
                row.cells[3].text = format_currency(item.get("total", 0))
        ...
        if price_list == "413":
            ...
            total_row.cells[0].text = "Số tiền phát sóng"
            total_row.cells[3].text = format_currency(total)
    ...
    def _add_total_and_words(self, doc: Document, items: List[Dict[str, Any]], price_list: str):
        ...
        p = doc.add_paragraph()
        p.add_run(f"(Bằng chữ: {in_words} đồng)")

        if price_list != "413":
            p2 = doc.add_paragraph()
            p2.add_run("- Giá trên đã bao gồm VAT.")

        doc.add_paragraph()

    def _add_contact(self, doc: Document):
        p = doc.add_paragraph()
        p.add_run("Xin vui lòng liên hệ: Phòng Dịch vụ")
        doc.add_paragraph()

        p = doc.add_paragraph()
        p.add_run("Địa chỉ: Số 2 Nguyễn Bình, phường Lê Chân, Hải Phòng")
        doc.add_paragraph()

        p = doc.add_paragraph()
        p.add_run("Điện thoại: 0912.023.771")
    ...
    def _add_signature(self, doc: Document):
        ...
        p.add_run("KT.GIÁM ĐỐC")
        doc.add_paragraph()
        doc.add_paragraph()
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        p.add_run("PHÓ GIÁM ĐỐC")
        doc.add_paragraph()
        doc.add_paragraph()
        doc.add_paragraph()
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        p.add_run("(Ký và ghi rõ họ tên)")
